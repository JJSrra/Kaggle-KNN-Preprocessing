...
#train[,1:50] = as.data.frame(scale(train[,1:50], center = mins, scale = maxs - mins))
maxs = apply(train[,1:50], 2, max)
mins = apply(train[,1:50], 2, min)

#train[,1:50] = apply(train[,1:50], 1, function(row) {
#    row = row - mins
#    row = row / (maxs - mins)
#})
  
#test[,1:50] = apply(test[,1:50], 1, function(row) {
#  row = row - mins
#  row = row / (maxs - mins)
#})
...
preprocess.params = preProcess(train[,1:50], c("center", "scale"))
train[,1:50] = predict(preprocess.params, train[,1:50])

# Train KNN models with odd Ks from 11 to 15 Ks to detect the best configuration
control = trainControl(method="cv", number=5)
knn.model = train(train[,1:50], train$C, method="knn",
...
write.table(output, file="submission-4.csv", quote=FALSE, sep=",", row.names=FALSE)
