...
# Feature selection
correlationMatrix = cor(train[,1:50])
...
preprocess.params = preProcess(train[,1:num.features], c("center", "scale"))
train[,1:num.features] = predict(preprocess.params, train[,1:num.features])

# Train KNN models with odd Ks from 11 to 15 Ks to detect the best configuration
control = trainControl(method="cv", number=5)
knn.model = train(train[,1:num.features], train$C, method="knn",
...
write.table(output, file="submission-5.csv", quote=FALSE, sep=",", row.names=FALSE)
